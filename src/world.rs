...
use crate::resources::{blocks, BlockState};

use self::chunks::{block_pos_to_index, BlockIndex, Chunk, ChunkSection};
...
impl World {
    ...
    */
    ...
    pub fn get_section(&self, location: &SectionLocation) -> Option<&ChunkSection> {
        self.get_chunk(&ChunkLocation::new(location.x, location.z))
            .map(|c| c.get_section(location.y))
            .unwrap_or(None)
    }

    pub fn get_section_mut(&mut self, location: &SectionLocation) -> Option<&mut ChunkSection> {
        self.get_chunk_mut(&ChunkLocation::new(location.x, location.z))
            .map(|c| c.get_section_mut(location.y))
            .unwrap_or(None)
    }

    pub fn get_section_containing(&self, coords: &WorldCoords) -> Option<&ChunkSection> {
    ...
    pub fn handle_block_change(&mut self, pack: PlayBlockChangeSpec) {
        if pack.block_id.0 < 0 || pack.block_id.0 >= blocks().len() as i32 {
            tracing::error!("Got block change with invalid block ID");
            return;
        }
        ...
        if let Some(chunk) = self.get_chunk_containing_mut(&coords) {
            ...
            tracing::warn!("Block change in unloaded chunk");
        }
    ...
    pub fn handle_multi_block_change(&mut self, pack: PlayMultiBlockChangeSpec) {
        ...
        if !self.is_chunk_loaded(&IVec2::new(loc.x, loc.z)) {
            tracing::error!("Got multi-block change in unloaded chunk: {:?}", loc);
            return;
        }
...
pub fn block_coords(pos: &DVec3) -> IVec3 {
    IVec3::new(
        pos.x.floor() as i32,
        pos.y.floor() as i32,
        pos.z.floor() as i32,
